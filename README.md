...
`flower adopt <existing-project>` remains read-only. Without `--dry-run`, it inspects conventional JavaScript/TypeScript and web-stack evidence, npm/pnpm/Yarn/Bun declarations and lockfiles, Supabase/PostgreSQL/Prisma paths, CI workflows, agent instructions, and Git worktree state. With `--dry-run`, it produces a deterministic, digest-protected adoption plan and still creates no files. The plan classifies every inspected existing file as project-owned with a never-overwrite policy, excludes local build and dependency roots (`.git`, `.next`, `coverage`, `dist`, and `node_modules`), and records exact file digests as application preconditions. Scans are bounded to 10,000 files, 5 MiB per file, and 64 MiB total. Symbolic links and requested adapters that overlap existing CI or agent instructions remain blocking conflicts. npm is the first supported adoption package manager; planning accepts optional comma-separated `--modules` and `--adapters` selections, while module resolution and transactional application remain later F7 slices.
