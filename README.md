...
- deterministic framework release resolution with channel and installed-module compatibility enforcement;
...
npm run flower -- adopt ../existing-app --inspect --json
npm run flower -- adopt ../existing-app --dry-run --id existing-app --name "Existing App" --json
npm run flower -- adopt ../existing-app --modules rbac --adapters codex --json
...
`flower adopt <existing-project>` applies one composed adoption transaction; use `--inspect` for the read-only evidence report or `--dry-run` for the deterministic, digest-protected plan. The plan classifies every inspected existing file as project-owned with a never-overwrite policy, resolves selected modules from a verified catalog, binds package and rendered-file digests, and binds every selected adapter output and state digest before writes. Application rejects changed project bytes or composition inputs, validates the planned project and ownership manifests, writes new `.flower` metadata, applies module and adapter engines beneath one outer rollback boundary, and records started and completed local journals. A failure after module or adapter success still removes every generated path and all control state. A repeated equivalent adoption is a verified no-op, and the immutable adoption record remains valid after legitimate later module or framework metadata changes. Local build and dependency roots (`.git`, `.next`, `coverage`, `dist`, and `node_modules`) are excluded; scans are bounded to 10,000 files, 5 MiB per file, and 64 MiB total. Symbolic links and requested adapters that overlap existing CI or agent instructions remain blocking conflicts. npm is the first supported adoption package manager. `--modules` and `--adapters` accept comma-separated selections, and `--catalog` can choose a verified local module catalog.
...
Phase F2 initializes a thin application shell. Phase F3 validates module manifests and transactionally adds, removes, or ejects generated integrations. Phase F4 now has a verified migration registry, versioned descriptors, deterministic planning, transactional PostgreSQL execution and verification, a node-postgres pool adapter, an RLS inspector, explicit read policies, actor/tenant scenarios, narrow authorization workflows, final-owner enforcement, append-only sensitive-field-safe audit insertion, a deterministic offline security gate, durable rate limits and quotas with bounded retention, fail-closed upload inspection orchestration, a narrow service-role boundary, redaction-safe structured logging, lockfile integrity and license policy checks, immutable CI actions, a live CI dependency-audit threshold, and hosted PostgreSQL certification. Phase F5 is complete: typed workflows, engine-owned approval, declared-effect and ownership enforcement, durable resumable journals, deterministic Codex and Claude generation, a constrained GitHub Actions verification projection, adapter drift validation, transactional materialization, and CLI synchronization satisfy the [F5 exit audit](docs/audits/phase-f5-exit.md). Phase F6 is complete: deterministic framework version resolution, digest-protected update plans, byte-aware generated-text three-way merging, consecutive schema-manifest migrations, exact forward module-configuration migration chains with source and definition drift checks, and a fixture-driven 0.1-to-0.2 scenario matrix satisfy the [F6 exit audit](docs/audits/phase-f6-exit.md). F7 is underway: inspection, deterministic planning, transactional base adoption, and composed selected-module/adapter adoption are implemented; user-facing transactional update application remains. The approved later roadmap assigns distribution and release experience to F8, deployment and hosted Supabase integration to F9, production hardening to F10, and the module/adapter ecosystem to F11. An external pilot may develop alongside them but remains a separate project and does not redefine framework scope.
