...
export const AGENT_ADAPTER_GENERATOR_VERSION = "1.1.0";
export const AGENT_ADAPTER_STATE_PATH = ".flower/generated/agent-adapters.json" as const;
export const GITHUB_ACTIONS_ADAPTER_PATH = ".github/workflows/flower-generated.yml" as const;
...
const CHECK_COMMANDS: Readonly<Record<string, string>> = {
  tests: "npm test",
  typecheck: "npm run typecheck",
  lint: "npm run lint --if-present",
  architecture: "npm run flower -- validate .",
  security: "npm run flower -- security check"
};
...
function workflowChecks(workflows: WorkflowDefinition[]): string[] {
  const checks: string[] = [];
  for (const workflow of workflows) {
    for (const step of workflow.steps) {
      if (step.action === "checks.run") checks.push(...(step.checks ?? []));
    }
  }
  return [...new Set(checks)];
}

function renderGithubActionsAdapter(inputDigest: string, checks: string[], missing: string[]): string {
  const metadata = JSON.stringify({
    schemaVersion: 1,
  ...
  const lines = [
    `# flower-agent-adapter ${metadata}`,
    "# Generated by Flower. Do not edit this file directly.",
    "# Verification-only projection: approvals, delegation, writes, and other workflow effects are never executed here.",
    ...(missing.length === 0 ? [] : [`# Blocking unsupported checks: ${missing.join(", ")}`]),
    "",
    "name: Flower generated verification",
    "",
    '"on":',
    "  push:",
    "    branches: [main]",
    ...
    "",
    "concurrency:",
    "  group: flower-generated-${{ github.workflow }}-${{ github.ref }}",
...
export function createAgentAdapterBundle(inputValue: AgentAdapterInput): AgentAdapterBundle {
  ...
  if (enabled.length > 0) assertProjectNotes(input);
  ...
  if (input.project.adapters?.githubActions === true) {
    assertGeneratedOutput(input, GITHUB_ACTIONS_ADAPTER_PATH);
    const checks = workflowChecks(input.workflows);
    const missing = checks.filter((check) => CHECK_COMMANDS[check] === undefined).map((check) => `check:${check}`).sort();
    const adapterInputDigest = digest({
      input,
      adapter: "github-actions",
      generatorVersion: AGENT_ADAPTER_GENERATOR_VERSION,
      projection: "checks.run",
      checkCommands: CHECK_COMMANDS
    });
    const content = renderGithubActionsAdapter(adapterInputDigest, checks, missing);
    artifacts.push({
      adapter: "github-actions",
      path: GITHUB_ACTIONS_ADAPTER_PATH,
      generatorVersion: AGENT_ADAPTER_GENERATOR_VERSION,
      inputDigest: adapterInputDigest,
      outputDigest: sha256(content),
      content,
