...
`flower adopt <existing-project>` now applies a base adoption transaction; use `--inspect` for the original read-only evidence report or `--dry-run` for the deterministic, digest-protected plan. The plan classifies every inspected existing file as project-owned with a never-overwrite policy, excludes local build and dependency roots (`.git`, `.next`, `coverage`, `dist`, and `node_modules`), and records exact file digests as application preconditions. Application rejects changed project bytes, validates the planned project and ownership manifests, writes only new `.flower` metadata, records started and completed local journals, verifies every written digest, and removes all created control state on failure. A repeated equivalent adoption is a verified no-op, and the immutable adoption record remains valid after legitimate later module or framework metadata changes. Scans are bounded to 10,000 files, 5 MiB per file, and 64 MiB total. Symbolic links and requested adapters that overlap existing CI or agent instructions remain blocking conflicts. npm is the first supported adoption package manager. Dry-run accepts optional comma-separated `--modules` and `--adapters` selections, but their composed transactional application remains the next F7 slice.
